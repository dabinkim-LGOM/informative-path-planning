...
class Tree(object):
    ...
    def rollout(self, current_node, belief):
        ...
        cumul_reward = 0.0 
        while cur_depth <= self.max_rollout_depth:
...
class MCTS(object):
    ...
    def get_actions(self):
        ...
        while time.clock() - time_start < self.budget:
            current_node = self.tree.get_next_child(current_node) #Find maximum UCT node (which is leaf node)
            iteration +=1
    ...
    def rollout_policy(self, node, budget):
        ...
        for i in xrange(self.max_rollout_depth):
            actions, _ = self.path_generator.get_path_set(node.pose) #plan from the last point in the sample
            ...
            #     a = np.random.randint(0,len(actions)) #choose a random path
            child = Node(pose=actions[a], parent=node, name=node.name+'_depth'+str(node.depth+1))
